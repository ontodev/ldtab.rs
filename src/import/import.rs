...
use serde_json::json; 
use horned_owl::model::{IRI, AnnotatedComponent, Ontology, Component, Import, OntologyAnnotation, RcStr};
    ...
    for ann_axiom in ontology.iter() { 
        ...
        if !annotations.is_empty() {
            ...
            let inspect = match axiom {
                ...
                //two special cases:
                Component::Import(x) => translate_ontology_import(&x, &iri),
                Component::OntologyAnnotation(x) => translate_ontology_annotation(&x, &iri),
...
pub fn owl_2_thick(annotated_axiom : &AnnotatedComponent<RcStr>) -> Value {
